/**
 * Pure-JS shim for ExpoCryptoAES using the Web Crypto API (crypto.subtle).
 * Replaces the native module for Expo Go SDK 54 compatibility.
 * Does NOT use registerWebModule — avoids "cannot override host object" error.
 */
...
class SealedData {
  ...
  async ciphertext(options) {
    ...
    const ciphertextLength = includeTag ? taggedCiphertextLength : taggedCiphertextLength - this.tagSize;
...
const AesCryptoModule = {
  ...
  async encryptAsync(plaintext, key, options = {}) {
    ...
    const gcmParams = aad ? { ...baseParams, additionalData: binaryInputBytes(aad) } : baseParams;
  ...
  async decryptAsync(sealedData, key, options = {}) {
    ...
    const baseParams = { name: "AES-GCM", iv, tagLength: sealedData.tagSize * 8 };
    const gcmParams = aad ? { ...baseParams, additionalData: binaryInputBytes(aad) } : baseParams;
...
export default AesCryptoModule;
