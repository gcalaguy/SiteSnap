...
export async function buildAuditPackage(opts: BuildOptions): Promise<PackageMeta> {
  ...
  try {
    ...
    // ── 7b. Fetch Pre-Inspection (PSI) checklists ─────────────────────────
    const psiConditions = [eq(psiChecklistsTable.companyId, companyId), eq(psiChecklistsTable.status, "submitted")];
    if (periodStart) psiConditions.push(gte(psiChecklistsTable.createdAt, new Date(periodStart)));
    if (periodEnd)   psiConditions.push(lte(psiChecklistsTable.createdAt, new Date(periodEnd)));
    if (projectIds?.length) psiConditions.push(inArray(psiChecklistsTable.projectId, projectIds));

    const psiChecklists = await db
      .select({
        ...
        submitterFirst: usersTable.firstName,
        submitterLast: usersTable.lastName,
        submittedAt: psiChecklistsTable.submittedAt,
      })
      ...
      .where(and(...psiConditions))
      .orderBy(desc(psiChecklistsTable.createdAt));

    const psiIds = psiChecklists.map((p) => p.id);
    const [psiSignatureRows, psiApprovalRows] = psiIds.length
      ? await Promise.all([
          db
            .select({
    ...
    const psiSignaturesByPsi = new Map<number, typeof psiSignatureRows>();
    for (const row of psiSignatureRows) {
      if (!psiSignaturesByPsi.has(row.psiId)) psiSignaturesByPsi.set(row.psiId, []);
      psiSignaturesByPsi.get(row.psiId)!.push(row);
    }
    const psiApprovalsByPsi = new Map<number, typeof psiApprovalRows>();
    for (const row of psiApprovalRows) {
      if (!psiApprovalsByPsi.has(row.psiId)) psiApprovalsByPsi.set(row.psiId, []);
    ...
    // ── 8b2. Pre-Inspection (PSI) Checklists folder ──
    const psiFolder = root.folder("PRE_INSPECTION_CHECKLISTS")!;
    if (psiChecklists.length > 0) {
      const psiCsvRows = psiChecklists.map((p) => {
        const hazards = p.hazards as Record<string, { checked?: string[] }>;
        const flaggedCategories = Object.entries(hazards ?? {})
          .filter(([, v]) => (v?.checked?.length ?? 0) > 0)
          .map(([k]) => k)
          .join("; ");
        return {
          PSI_ID: p.id,
          ...
          Trade_Task: p.tradeDescription ?? "",
          Location: p.location ?? "",
          Weather: p.weatherTemp ?? "",
          Created_By: p.submitterFirst ? `${p.submitterFirst} ${p.submitterLast}` : "",
          Submitted_At: formatDate(p.submittedAt as Date),
          Hazard_Categories_Flagged: flaggedCategories,
          Signature_Count: (psiSignaturesByPsi.get(p.id) ?? []).length,
          Approval_Count: (psiApprovalsByPsi.get(p.id) ?? []).length,
        };
      });
      addFile(psiFolder, "psi_checklists.csv", toCsv(psiCsvRows));

      const psiFullJson = psiChecklists.map((p) => ({
        id: p.id,
        project: p.projectName,
        date: formatDate(p.date),
        ...
        createdBy: p.submitterFirst ? `${p.submitterFirst} ${p.submitterLast}` : "",
        submittedAt: formatDate(p.submittedAt as Date),
        hazards: p.hazards,
        taskRows: p.taskRows,
        voiceNotes: p.voiceNotes,
        signatures: (psiSignaturesByPsi.get(p.id) ?? []).map((s) => ({
          name: s.firstName ? `${s.firstName} ${s.lastName}` : "",
          signedAt: formatDate(s.signedAt as Date),
        })),
        approvals: (psiApprovalsByPsi.get(p.id) ?? []).map((a) => ({
          name: a.firstName ? `${a.firstName} ${a.lastName}` : "",
          approvedAt: formatDate(a.approvedAt as Date),
        })),
      }));
      addFile(psiFolder, "psi_checklists_full.json", JSON.stringify(psiFullJson, null, 2));
    } else {
      addFile(psiFolder, "NO_PSI_CHECKLISTS.txt",
