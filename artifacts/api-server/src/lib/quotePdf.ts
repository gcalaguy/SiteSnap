...
export function buildQuotePdfBuffer(input: QuotePdfInput): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    // Fixed page dimensions (LETTER size doesn't change across pages) — used to
    // decide when content needs to flow onto a new page instead of being drawn
    // past the visible page area (where pdfkit would silently drop it).
    const pageH = doc.page.height;
    const BOTTOM_MARGIN = 60; // reserve room for the footer band

    function drawFooter() {
      doc.fillColor(DARK).rect(0, pageH - 30, doc.page.width, 30).fill(DARK);
      doc.fillColor(FOOTER_GRAY).fontSize(8).font("Helvetica");
      doc.text(`Generated by Site Snap · ${input.quoteNumber}`, 50, pageH - 14);
      doc.text(
    ...
    doc.text("QUOTE", doc.page.width - 50, 28, { align: "right" });
    doc.fontSize(14).font("Helvetica-Bold").fillColor(WHITE);
    doc.text(input.quoteNumber, doc.page.width - 50, 48, { align: "right" });

    let y = 100;

    // Breaks to a new page (finalizing the current page's footer first) when the
    // next block wouldn't fit above the footer band. Without this, pdfkit just
    // draws past the page's visible area and the content is silently lost.
    ...
    doc.text("VALID UNTIL", rightX, y, { align: "right" });
    ...
    const validUntil = input.validUntil
      ? new Date(input.validUntil).toLocaleDateString("en-CA", { year: "numeric", month: "short", day: "numeric" })
      : "—";
    doc.text(issueDate, rightX - 120, y + 14, { align: "right" });
    doc.text(validUntil, rightX, y + 14, { align: "right" });
    ...
    input.lineItems.forEach((item, i) => {
      ensureSpace(18);
      if (y === 50) drawTableHeader(); // continued onto a new page — repeat the header
    ...
    // Notes
    if (input.notes) {
      const notesHeight = doc.heightOfString(input.notes, { width: doc.page.width - 100 });
      ensureSpace(14 + notesHeight + 10);
      doc.fillColor(GRAY).fontSize(8).font("Helvetica-Bold");
      doc.text("NOTES", 50, y);
      y += 14;
      doc.fillColor(DARK).fontSize(10).font("Helvetica");
    ...
    if (input.defaultNotes) {
      const termsHeight = doc.heightOfString(input.defaultNotes, { width: doc.page.width - 100 });
      ensureSpace(14 + termsHeight + 10);
      doc.fillColor(GRAY).fontSize(8).font("Helvetica-Bold");
    ...
    // Footer (final page — earlier pages already got theirs from ensureSpace)
    drawFooter();
