...
const ToQuoteBody = z.object({
  ...
  clientCompanyName: z.string().max(300).optional(),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const {
      title, clientName, clientEmail, clientCompanyName, clientAddress, clientPhone,
      notes, sourcePrompt, source, validUntil, lineItems,
    } = parsed.data;
